...
- **The in-memory `#[Version]` property bump is deferred until after the flush transaction commits.** `QueryExecutor::executeUpdate()` and `ChangeSetExecutor::executeSoftDelete()` return the `+1` write as a `\Closure`; `ChangeSetExecutor::execute()` collects them and `EntityManager::flush()` invokes them only after `commit()`, then `clearChanges()` refreshes the snapshot from the bumped value. A throw or rollback simply drops the closures, so a mid-flush conflict never leaves a successfully-updated sibling's property ahead of its row — the retry the feature exists to enable still matches. Consequences: (a) the DB-side `SET version = version + 1` still runs inline, so writing one row through two `#[Version]`-checking classes in a single flush still self-conflicts on the second UPDATE; (b) `#[PostUpdate]`/post-flush callbacks run before commit, so they observe the pre-bump version value; (c) when `flush()` runs inside a caller-owned transaction it does not commit and the bumps are applied at flush end regardless — a caller rollback then strands them, so use `flush()`'s own transaction (or `EntityManager::transactional()`) for rollback-safe retries.
