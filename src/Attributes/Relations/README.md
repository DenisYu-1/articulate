...
- Column name on main side: snake_case(property) + `_id` by default; override with `column: 'custom_fk'`; nullable follows PHP type (`?Foo` → nullable); when multiple entities map the same table/column the nullable flag is relaxed if any entity allows nulls
