...
class QueryBuilderDmlTest extends DatabaseTestCase {
    ...
    public function testInsertSingleEntity(string $databaseName): void
    {
        ...
        $this->connection->executeQuery('CREATE TABLE test_users (id INT AUTO_INCREMENT PRIMARY KEY, name VARCHAR(255), email VARCHAR(255))');
        ...
        $result = $this->connection->executeQuery('SELECT * FROM test_users WHERE id = ?', [$id])->fetchAll();
    ...
    public function testInsertMultiRow(string $databaseName): void
    {
        ...
        $results = $this->connection->executeQuery('SELECT * FROM test_users ORDER BY id')->fetchAll();
    ...
    public function testInsertWithNullValues(string $databaseName): void
    {
        ...
        $this->connection->executeQuery('CREATE TABLE test_users (id INT AUTO_INCREMENT PRIMARY KEY, name VARCHAR(255) NOT NULL, email VARCHAR(255) DEFAULT NULL)');
    ...
    public function testUpdateByEntity(string $databaseName): void
    {
        ...
        $this->connection->executeQuery('CREATE TABLE test_users (id INT AUTO_INCREMENT PRIMARY KEY, name VARCHAR(255), email VARCHAR(255))');
        $this->connection->executeQuery('INSERT INTO test_users (id, name, email) VALUES (1, ?, ?)', ['John', 'john@example.com']);
        ...
        $result = $this->connection->executeQuery('SELECT * FROM test_users WHERE id = 1')->fetchAll();
        $this->assertCount(1, $result);
    ...
    public function testUpdateWithCustomWhere(string $databaseName): void
    {
        ...
        $this->connection->executeQuery('CREATE TABLE test_users (id INT AUTO_INCREMENT PRIMARY KEY, name VARCHAR(255), email VARCHAR(255))');
        $this->connection->executeQuery('INSERT INTO test_users (id, name, email) VALUES (1, ?, ?), (2, ?, ?)', ['John', 'john@example.com', 'Jane', 'jane@example.com']);

        $affected = $this->qb->update('test_users')->set('email', 'updated@example.com')->where('id IN (?)', [1, 2])->execute();

        $this->assertEquals(2, $affected);

        $results = $this->connection->executeQuery('SELECT * FROM test_users WHERE id IN (1, 2)')->fetchAll();
    ...
    public function testUpdateWithSetArray(string $databaseName): void
    {
        ...
        $affected = $this->qb->update('test_users')->set(['name' => 'John Updated', 'email' => 'updated@example.com'])->where('id = ?', 1)->execute();
    ...
    public function testDeleteByEntity(string $databaseName): void
    {
        ...
        $results = $this->connection->executeQuery('SELECT * FROM test_users')->fetchAll();
    ...
    public function testDeleteWithCustomWhere(string $databaseName): void
    {
        ...
        $affected = $this->qb->delete('test_users')->where('id = ?', 1)->execute();
    ...
    public function testInsertReturningPostgreSQL(string $databaseName): void
    {
        ...
        $this->connection->executeQuery('CREATE TABLE test_users (id SERIAL PRIMARY KEY, name VARCHAR(255), email VARCHAR(255))');
    ...
    public function testUpdateReturningPostgreSQL(string $databaseName): void
    {
        ...
        $this->connection->executeQuery('CREATE TABLE test_users (id SERIAL PRIMARY KEY, name VARCHAR(255), email VARCHAR(255))');
        $this->connection->executeQuery('INSERT INTO test_users (name, email) VALUES (?, ?)', ['John', 'john@example.com']);

        $result = $this->qb->update('test_users')->set('name', 'John Updated')->where('id = ?', 1)->returning('id', 'name')->execute();
    ...
    public function testDeleteReturningPostgreSQL(string $databaseName): void
    {
        ...
        $result = $this->qb->delete('test_users')->where('id = ?', 1)->returning('id', 'name')->execute();
    ...
    public function testInsertWithTableName(string $databaseName): void
    {
        ...
        $id = $this->qb->insert('test_users')->values(['name' => 'John', 'email' => 'john@example.com'])->execute();
    ...
    public function testResetClearsDmlState(string $databaseName): void
    {
        ...
        $this->connection->executeQuery('CREATE TABLE test_users (id INT AUTO_INCREMENT PRIMARY KEY, name VARCHAR(255))');
        ...
        $sql = $this->qb->select('*')->from('test_users')->getSQL();
        $this->assertEquals('SELECT * FROM test_users', $sql);
    }
}
